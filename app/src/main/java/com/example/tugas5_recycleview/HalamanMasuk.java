...
import com.example.tugas5_recycleview.databinding.ActivityHalamanMasukBinding;
import android.widget.Toast;

public class HalamanMasuk extends AppCompatActivity implements
        View.OnClickListener {
    MyDbHelper myDbHelper;
    private ActivityHalamanMasukBinding binding;
    private SessionManager session;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        //setup view binding
        binding = ActivityHalamanMasukBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        session = new SessionManager(getApplicationContext());
        if (session.isLoggedIn()) {
            Intent intent = new Intent(HalamanMasuk.this, DestinationActivity.class);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.Signinbuttonid) {
            ...
            Intent intent = new Intent(HalamanMasuk.this, RegisterActivity.class);
            startActivity(intent);
        }
    }
}
